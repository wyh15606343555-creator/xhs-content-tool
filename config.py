...
TONE_STYLES = [
    "专业权威",      # 数据+术语，适合医美/教育
    "温暖亲切",      # 朋友聊天感，适合母婴/生活
    "活泼俏皮",      # emoji多、语气词，适合美妆/零食
    "闺蜜种草",      # "姐妹们！"开头，真实体验感，小红书第一爆款风格
    "反差吐槽",      # "本来没抱希望…结果？！"，高点击率
    "干货清单",      # "5个方法/3步搞定"，高收藏率
    "高级简约",      # 短句+留白，适合轻奢/设计
]
...
# ═══════════════════════════════════════════════════════
ANALYZE_PROMPT = (
    "你是小红书爆款内容分析专家。分析以下小红书笔记，找出它为什么能火。\n\n"
    "笔记内容：\n{note_content}\n\n"
    "请用JSON格式返回分析结果，包含以下字段：\n"
    "- hooks: 数组，列出标题和开头使用的吸引注意力的手法（如悬念、数字、情绪词）\n"
    "- structure: 字符串，描述内容结构（如\"痛点→方案→效果\"或\"清单→对比→推荐\"）\n"
    "- emotional_triggers: 数组，列出触发用户互动的情绪点（如焦虑、好奇、共鸣、羡慕）\n"
    "- weaknesses: 数组，列出可以改进的地方（如缺少具体数据、图文不匹配、CTA弱）\n\n"
    "只返回JSON，不要其他文字。"
)

STRATEGY_PROMPT = (
    "你是小红书内容策略师。基于竞品分析结果，制定一篇新笔记的内容策略。\n\n"
    "竞品分析：\n{analysis_json}\n\n"
    "{store_section}\n\n"
    "发帖目的：{post_goal}\n"
    "语气风格：{tone_style}\n"
    ...
    "请用JSON格式返回策略，包含以下字段：\n"
    "- angle: 字符串，这篇笔记的切入角度（要与竞品形成差异）\n"
    "- differentiators: 数组，与竞品的差异化卖点（如果没有店铺资料则基于行业通用优势）\n"
    "- structure_plan: 字符串，推荐的内容结构（如\"个人体验→产品测评→购买建议\"）\n"
    "- tone_guide: 字符串，具体的语气指导（基于选择的语气风格，给出用词和句式建议）\n\n"
    "只返回JSON，不要其他文字。"
)

POLISH_PROMPT = (
    "你是小红书文案润色专家。用户已经手动编辑了一篇小红书笔记，请在保留用户修改意图的前提下优化文案。\n\n"
    "语气风格要求：{tone_style}\n\n"
    "当前标题：\n{title}\n\n"
    "当前正文：\n{body}\n\n"
    "优化方向：\n"
    "1. 保留用户的核心内容和关键信息不变\n"
    "2. 优化小红书风格表达（口语化、有感染力）\n"
    "3. 合理使用emoji增加视觉节奏\n"
    "4. 优化段落结构和阅读流畅度\n"
    "5. 标题要有吸引力，适合小红书平台\n\n"
    "请用JSON格式返回，包含 title 和 body 两个字段。只返回JSON，不要其他文字。"
)
